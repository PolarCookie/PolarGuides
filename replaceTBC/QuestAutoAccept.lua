...
function me:QUEST_GREETING()
	...
	if self.db.profile.autoaccept then
		for qnum=1,GetNumAvailableQuests() do
			for i,goal in ipairs(self.CurrentStep.goals) do
				if goal.action=="accept" and goal.quest==GetAvailableTitle(qnum) and goal:GetStatus()=="incomplete" and not goal.noautoaccept then
...
function me:GOSSIP_SHOW()
	...
	if self.db.profile.autoaccept then
		...
		for qnum=1,GetNumGossipAvailableQuests() do
			for i,goal in ipairs(self.CurrentStep.goals) do
				if goal.action=="accept" and goal.quest==quests[qnum*3-2] and goal:GetStatus()=="incomplete" and not goal.noautoaccept then
					self:Print("Opening quest")
...
function me:QUEST_DETAIL()
	...
	if self.db.profile.autoaccept then
		...
		for i,goal in ipairs(self.CurrentStep.goals) do
			if goal.action=="accept" and goal.quest==title and goal:IsCompleteable() and goal:GetStatus()~="obsolete" and not goal.noautoaccept then
